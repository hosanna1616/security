...
export default function ReportsPage() {
  ...
  const getMonthlyStats = () => {
    ...
    return months.map((month, index) => {
      ...
      const totalDownloads = monthDownloads.reduce((sum, download) => sum + download.count, 0)
  ...
  const getProductStats = () => {
    const products = ['Gasha Antivirus', 'Gasha VPN', 'Gasha WAF', 'Nisir SIEM']
    
    return products.map(product => {
      const productDownloads = downloads.filter(download => 
        download.file.toLowerCase().includes(product.toLowerCase().replace(' ', ''))
      )
      
      const productRequests = requests.filter(request => 
        request.type?.toLowerCase().includes(product.toLowerCase().split(' ')[1] || product.toLowerCase())
      )

      const totalDownloads = productDownloads.reduce((sum, download) => sum + download.count, 0)
  ...
  const getPerformanceMetrics = () => {
    const totalDownloads = downloads.reduce((sum, download) => sum + download.count, 0)
    ...
    const productStats = getProductStats()
    const mostDownloaded = productStats.reduce((prev, current) => 
      (prev.downloads > current.downloads) ? prev : current, { product: 'None', downloads: 0 }
    )
    ...
    <div className='p-4 sm:p-6 min-h-screen bg-gradient-to-br from-gray-900 to-gray-800'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8'>
        <div className='bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl border border-gray-700/30'>
          ...
          <p className='text-3xl font-bold text-blue-400'>{performanceMetrics.totalDownloads}</p>
        ...
        <div className='bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl border border-gray-700/30'>
          ...
          <p className='text-3xl font-bold text-green-400'>{performanceMetrics.totalRequests}</p>
        ...
        <div className='bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl border border-gray-700/30'>
          ...
          <p className='text-3xl font-bold text-emerald-400'>{performanceMetrics.approvalRate}%</p>
        ...
        <div className='bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl border border-gray-700/30'>
          ...
          <p className='text-3xl font-bold text-yellow-400'>{performanceMetrics.pendingRequests}</p>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8'>
        ...
        <div className='bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl border border-gray-700/30'>
          ...
          <ResponsiveContainer width='100%' height={300}>
            <BarChart data={monthlyStats}>
              ...
              <Legend />
              <Bar dataKey='downloads' name='Downloads' fill='#4F46E5' radius={[4, 4, 0, 0]} />
        ...
        <div className='bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl border border-gray-700/30'>
          <h2 className='text-xl font-semibold text-white mb-4'>Request Status Distribution</h2>
          <ResponsiveContainer width='100%' height={300}>
            <PieChart>
              <Pie
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8'>
        ...
        <div className='bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl border border-gray-700/30'>
          <h2 className='text-xl font-semibold text-white mb-4'>Download Distribution</h2>
          <ResponsiveContainer width='100%' height={300}>
            <PieChart>
              <Pie
      ...
      <div className='bg-gray-800/50 backdrop-blur-sm p-6 rounded-xl border border-gray-700/30'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4 text-sm'>
          <div>
            <h3 className='font-semibold text-gray-300 mb-2'>Downloads Information</h3>
            <p className='text-gray-400'>Total unique files: {downloads.length}</p>
            <p className='text-gray-400'>Most downloaded: {performanceMetrics.mostDownloaded}</p>
            <p className='text-gray-400'>Last updated: {new Date().toLocaleString()}</p>
          </div>
          <div>
            <h3 className='font-semibold text-gray-300 mb-2'>Requests Information</h3>
            <p className='text-gray-400'>Approval rate: {performanceMetrics.approvalRate}%</p>
            <p className='text-gray-400'>Pending reviews: {performanceMetrics.pendingRequests}</p>
            <p className='text-gray-400'>Total products: {productStats.length}</p>
          </div>
        </div>
      </div>
    </div>
