...
export default function SettingsPage() {
  ...
  const saveSecurity = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Current password entered:", securityForm.currentPassword);
      console.log("Stored password hash:", security.currentPasswordHash);
      console.log(
        "Entered password hash:",
        simpleHash(securityForm.currentPassword)
      );
      ...
      // Verify current password - FIXED: Use consistent hashing
      const enteredPasswordHash = simpleHash(securityForm.currentPassword);
      const storedPasswordHash =
        security.currentPasswordHash || DEFAULT_PASSWORD_HASH;

      console.log("Comparison:", {
        entered: enteredPasswordHash,
      ...
      if (enteredPasswordHash !== storedPasswordHash) {
        setMessages({
          type: "error",
          text: "Current password is incorrect. Default password is 'admin123'",
        });
        setTimeout(() => setMessages(null), 3000);
        return;
      }
